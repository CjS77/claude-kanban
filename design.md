...
- [x] One search box over the whole board, plus the epic dropdown (a discovery affordance for ids and titles nobody memorises; it ANDs
  ...
  The box sits after the create buttons, wearing a magnifier and a `?` that opens the grammar as a `<details class="dropdown">` —
  daisyUI exempts `<details>` from its dropdown hide rule, so the reference costs no JavaScript. `#filters` wraps the create
  buttons too, purely so both filter controls stay inside the `hx-include` target with the buttons between them; htmx harvests
  only `input`/`textarea`/`select` from an include target, so the buttons stay invisible to it
- Templates may only use classes already in `assets/app.css`. It is a committed Tailwind build, regenerated by hand with
  `just css`, so a class nobody compiled is silently dead — `w-72` sat inertly on the search box for a release. Where the build
  lacks a utility (a gap wider than `gap-4`, an explicit width, `dropdown-end`), the header uses an inline `style` instead,
  the same way the repo mark is sized by SVG attributes
