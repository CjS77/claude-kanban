...
- Four-column board: create, edit, drag, and delete tickets and epics; one search box narrows it — `landed: true, label: ux, realtime
  ...
  are `text:` `label:` `epic:` `id:` `note:` `status:` `col:` `model:` `effort:` `landed:` `discarded:` `blocked:` `auto-merge:`.
  `epic:none` (or
...
- Per-ticket **model and effort**: a ticket can say what its work is worth running at — `opus` and `xhigh` for the hairy refactor, nothing at all for the one-liner — and `/kanban:work` dispatches it that way instead of using whatever your session happens to be on. Set both on the card; leave them blank to inherit. The suggested vocabulary is the board's `"models"` config, defaulting to the Claude aliases. The board can also set `"implement_model"` and `"refine_model"` — the model that works a card naming none of its own, split between writing code and writing specs; a ticket's own `model` always wins
- Opt-in **auto-merge**: flag a ticket (or a whole epic) and `/kanban:work` rebases its branch onto main and fast-forwards it in when the ticket reaches review, instead of handing the branch back to you. It creates the evidence the landing sweep already looks for rather than skipping it, so the card still only reaches done by proof — off by default, and it only ever runs inside a work loop you started
- Opt-in **minesweeper delegation** (`"minesweeper": true`): kanban writes no code — a ready ticket entering doing is mirrored to a GitHub issue wearing `"minesweeper_label"` (default `autofix`) for an external [minesweeper](https://github.com/CjS77/minesweeper) daemon; the poller finds the daemon's PR through the issue's `Fixes #n` reference and moves the card to review, failures (`"minesweeper_flag_labels"`) flag the card in place, and a refine split mirrors sub-issues as child tickets. A **Hand to minesweeper** checkbox on the New-ticket modal delegates a single ticket on creation, toggle or no toggle. Off by default per project, and `cargo build --no-default-features` compiles the whole egress out
