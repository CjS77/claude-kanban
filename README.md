...
- Per-ticket **model and effort**: a ticket can say what its work is worth running at — `opus` and `xhigh` for the hairy refactor, nothing at all for the one-liner — and `/kanban:work` dispatches it that way instead of using whatever your session happens to be on. Set both on the card; leave them blank to inherit. The suggested vocabulary is the board's `"models"` config, defaulting to the Claude aliases
