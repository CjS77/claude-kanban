...
**Hand to minesweeper** checkbox doing the same for a single ticket at creation, toggle or no toggle. Use this command
...
> the server name — `kanban_board` appears in your tool list as `kanban_kanban_board`, and so on. Every `kanban_*`
> tool named below means the prefixed tool.
